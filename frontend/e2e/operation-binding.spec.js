...
test.describe('Operation binding', () => {
  test('changing operation type updates YAML', async ({ page }) => {
    ...
    const actionItems = page.locator('rr-list-item:has(rr-text-cell:has-text("resultaat"))');
    await actionItems.last().locator('rr-button:has-text("Bewerk")').click();
    await page.waitForTimeout(300);
  ...
  test('changing literal value updates YAML', async ({ page }) => {
    ...
    const value1Input = panel.locator('[data-testid="op-value-0"] rr-text-field input');
  ...
  test('removing a value via minus button updates YAML', async ({ page }) => {
    ...
    // Click minus button on first value (rr-icon-button may be "not visible" to Playwright)
    const removeBtn = panel.locator('[data-testid="op-value-0"] rr-icon-button[icon="minus"]');
